...
class Whisper(nn.Module):
    def __init__(self, model_name="openai/whisper-base"):
        ...
        self.proj_out = self.model.proj_out
    ...
    def encode(self, batch):
        encoding = self.encoder(
            batch.input_features, output_attentions=True,
            output_hidden_states=True, return_dict=True
        )[0]

        return WhisperOutput(encoding, logits=None)

    def decode(self, batch, encoding):
        return self.decoder(
            input_ids=batch.input_ids, attention_mask=batch.attention_mask,
            encoder_hidden_states=encoding, output_attentions=False,
            output_hidden_states=False, return_dict=True
        )[0]

    def forward(self, batch, encoding=None):
        if encoding is None:
            encoding = self.encode(batch).encoding

        decoded = self.decode(batch, encoding)
        logits = self.proj_out(decoded).permute(0, 2, 1)

        return WhisperOutput(encoding=encoding, logits=logits)
    ...
    def prepare_batch(self, batch):
        ...
        audio, input_features = None, None
        ...
        token_ids = [self.tokenizer.encode(x) for x in transcripts]
        token_ids = self.tokenizer.pad(
            [{"input_ids": x} for x in token_ids], max_length=448,
            padding='max_length', return_tensors='pt'
        )

        input_ids = token_ids.input_ids.cuda()
        attention_mask = token_ids.attention_mask.cuda()
        masked_input_ids = input_ids.masked_fill(attention_mask.ne(1), -100)

        return WhisperBatch(
            audio=audio, input_features=input_features, transcripts=transcripts,
            input_ids=input_ids, attention_mask=attention_mask,
            masked_input_ids=masked_input_ids
        )

    def generate(self, input_features):
        token_ids = self.model.generate(input_features)
